  ...
  return (
    ...
    >
      <CustomImage
        ...
        width={219}
        height={190}
        alt={translated_item?.name || "Product"}
        className="w-[100px] sm:w-[219px] h-auto aspect-square sm:aspect-219/190 rounded object-cover"
      />

      <div
        onClick={handleLikeItem}
        className="absolute size-6 sm:size-9 ltr:right-2 rtl:left-2 top-2 bg-white rounded-full flex items-center justify-center text-primary z-10"
      >
        {item?.is_liked ? (
          <button>
            <FaHeart className="size-4 sm:size-6" />
          </button>
        ) : (
          <button>
      ...
      <div className="flex flex-col gap-1 sm:gap-2 flex-1 relative min-w-0">
        ...
        <p
          ...
          title={translated_item?.name}
        >
          {translated_item?.name}
        </p>
