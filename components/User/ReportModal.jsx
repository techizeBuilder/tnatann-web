    ...
    <Dialog open={isReportModalOpen} onOpenChange={handleClose}>
      <DialogContent onInteractOutside={(e) => e.preventDefault()}>
          ...
          <div className="flex flex-col gap-2">
            ...
            {selectedReason === "other" && (
              <textarea
                ...
                className="w-full p-2 text-sm border rounded-md focus:outline-hidden focus:ring-1 focus:ring-primary mt-2"
                rows={3}
              />
            )}
          </div>
        ...
        <div className="flex gap-2 flex-col md:flex-row">
          <button
            className="w-full py-2 px-4 text-white  text-base font-medium  bg-black rounded-md hover:bg-primary/90 transition-colors"
            onClick={handleClose}
          >
            {t("cancel")}
          </button>
