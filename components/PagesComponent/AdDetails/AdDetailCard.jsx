...
const AdDetailCard = ({ productDetails, setProductDetails }) => {
  ...
  const headline = `🚀 Discover the perfect deal! Explore "${translated_item?.name || productDetails?.name
    ...
    <div className="flex flex-col gap-4 border p-4 rounded-lg">
      <div className="flex justify-between max-w-full">
        <div className="flex flex-col gap-2">
          <h1
            className="text-xl sm:text-2xl font-medium word-break-all line-clamp-2"
        ...
        <div className="flex flex-col gap-4">
          ...
          >
            {productDetails?.is_liked === true ? (
              <FaHeart className="size-4 sm:size-5 text-primary" />
            ) : (
