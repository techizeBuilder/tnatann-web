...
import CategorySelectToListAd from "./CategorySelectToListAd";
import {
  addItemApi,
  categoryApi,
  getCurrenciesApi,
...
import LocationSelectToListAd from "./LocationSelectToListAd";
...
const AdsListing = () => {
  ...
  const handleFullSubmission = () => {
    ...
    if (!validateSeoDetails(seoDetails, languages, defaultLangId)) {
      return setStep(4);
    }
  ...
  const postAd = async () => {
    ...
    const allData = {
      ...
      category_id: lastItemId,
      gallery_images: otherImages.map((imgObj) => imgObj.file),
      ...
      ...(seo_details?.length > 0 && { seo_details }),
  ...
  const handleTabClick = (tab) => {
    if (tab === 1 && !disabledTab.categoryTab) {
      ...
      setStep(5);
    } else if (tab === 6 && !disabledTab.location) {
      setStep(6);
    }
  };
        ...
        <div className="flex flex-col gap-8 mt-8">
          ...
          <div className="flex items-center gap-3 justify-between">
            ...
            <h1 className="text-2xl font-medium sm:hidden">
              {
                step == 2 ? t("addDetails") : step == 3 ? t('addExtraDetails') : step == 4 ? t('addImages') : step == 5 ? t("addLocation") : t("adListing")
              }
            </h1>

            {/* Mobile-only selector */}
            <div className="sm:hidden">
              {(step == 2 || (step === 3 && hasTextbox) || step === 4) && (
                <AdLanguageSelector
                  langId={langId}
                  setLangId={setLangId}
                  languages={languages}
              ...
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <div
                  className={`transition-all duration-300 p-2 cursor-pointer ${step === 4 ? "bg-primary text-white" : ""
                    } rounded-md  ${disabledTab?.metaTab == true ? "opacity-60" : " "
                    }`}
                ...
                <div
                  className={`transition-all duration-300 p-2 cursor-pointer ${step === 6 ? "bg-primary text-white" : ""
