...
export const GET_POPULAR_CATEGORIES = "get-popular-categories";
...
export const GET_REFER_POINTS_BALANCE = "refer-points-balance";
export const GET_USER_PURCHASED_PACKAGES = "get-user-purchased-packages";
export const GENERATE_META_DETAILS = "gemini/generate-meta";
export const GENERATE_DESCRIPTION = "gemini/generate-description";
export const GET_PAYMENT_RECEIPT = "get-payment-receipt";
export const GET_HOME_SCREEN = "get-home-screen";
...
// 1. Home Screen Sections Visibility API
export const homeScreenApi = {
  getHomeScreen: () => {
    return Api.get(GET_HOME_SCREEN);
  },
}
...
export const popularCategoriesApi = {
  getPopularCategories: () => {
    return Api.get(GET_POPULAR_CATEGORIES);
  },
};
// 3. MY ITEMS API
...
export const chatListApi = {
  chatList: ({ type, page, item_id, search } = {}) => {
...
export const addItemApi = {
  addItem: ({
    ...
    if (seo_details) {
      seo_details.forEach(({ language_id, ...fields }) => {
        Object.entries(fields).forEach(([key, value]) => {
          formData.append(`seo_details[${language_id}][${key}]`, value);
        });
      });
    }
...
export const editItemApi = {
  editItem: ({
    ...
    if (category_id) formData.append("category_id", category_id);
    if (price) formData.append("price", price);
    if (delete_item_image_id && Array.isArray(delete_item_image_id)) {
      delete_item_image_id.forEach((id) => {
        formData.append("delete_item_image_id[]", id); // Note the [] in the key
      });
    }
    if (contact) formData.append("contact", contact);
    if (video_link) formData.append("video_link", video_link);
...
export const getChatAdListApi = {
  getChatAdList: ({ type, page, search } = {}) => {
...
export const deleteChatApi = {
  deleteChat: ({ item_offer_id = [] } = {}) => {
    const formData = new FormData();
    item_offer_id.forEach((id) => {
      formData.append("item_offer_id[]", id);
    });
...
export const getReferralCodeApi = {
  getReferralCode: () => {
    return Api.get(GET_REFERRAL_CODE);
  },
};


export const getReferPointsBalanceApi = {
  getReferPointsBalance: () => {
    return Api.get(GET_REFER_POINTS_BALANCE);
  },
};


export const getUserPurchasedPackagesApi = {
  getUserPurchasedPackages: ({ type } = {}) => {
    return Api.get(GET_USER_PURCHASED_PACKAGES, {
      params: {
        type
      }
    });
...
export const generateMetaDetailsApi = {
  generateMetaDetail: ({
    title,
    price,
    currency_iso_code,
    ...
    if (price) formData.append("price", price);
    if (currency_iso_code) formData.append("currency_iso_code", currency_iso_code);
    if (language_id) formData.append("language_id", language_id);
    return Api.post(GENERATE_META_DETAILS, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
...
export const generateDescriptionApi = {
  generateDescription: ({ title, language_id, price, currency_iso_code } = {}) => {
    const formData = new FormData();
    if (title) formData.append("title", title);
    if (language_id) formData.append("language_id", language_id);
    if (price) formData.append("price", price);
    if (currency_iso_code) formData.append("currency_iso_code", currency_iso_code);
    return Api.post(GENERATE_DESCRIPTION, formData, {
      headers: { "Content-Type": "multipart/form-data" },
    });
  },
};

export const getPaymentReceiptApi = {
  getPaymentReceipt: ({ payment_transaction_id } = {}) => {
    return Api.get(GET_PAYMENT_RECEIPT, {
      params: { payment_transaction_id },
    });
  },
};