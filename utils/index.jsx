...
export const seoData = (options) => {
  const { seo, url } = options || {};
  const { title, description, keywords, image, schema } = seo || {};
  const { baseUrl, langCode, path, paramsStr, supportedLangs, defaultLangCode } = url || {};
  ...
  const setMeta = (selector, attrs, content) => {
    getMeta(selector, attrs).setAttribute("content", content);
  };

  // Build URLs if url options provided
  let canonicalUrl;
  let alternates;
  if (url && baseUrl && langCode && path && Array.isArray(supportedLangs) && defaultLangCode) {
    const queryStr = paramsStr ? `?${paramsStr}` : "";
    canonicalUrl = `${baseUrl}/${langCode}/${path}${queryStr}`;

    const languagesObj = supportedLangs.reduce((acc, lng) => {
      acc[lng] = `${baseUrl}/${lng}/${path}${queryStr}`;
      return acc;
    }, {});
    ...
    alternates = {
      languages: {
        ...languagesObj,
        "x-default": `${baseUrl}/${defaultLangCode}/${path}${queryStr}`,
      },
    };
  }
  ...
  // Update hreflangs — only remove nodes seoData itself created, never React-managed ones
  if (alternates?.languages) {
    document.querySelectorAll('link[rel="alternate"][hreflang][data-seo-managed]').forEach(link => link.remove());
  ...
  // Schema (JSON-LD) — body-scoped; finds any existing body JSON-LD (server or client)
  const existingSchema = document.body.querySelector('script[type="application/ld+json"]');
  let schemaContent = null;
  if (schema) {
    try {
      schemaContent = typeof schema === "string" ? (JSON.parse(schema), schema) : JSON.stringify(schema);
    } catch (e) {
      console.error("Invalid JSON for schema:", e);
    }
  }
  if (schemaContent) {
    if (existingSchema) {
      existingSchema.innerHTML = schemaContent;
    } else {
      const tag = document.createElement("script");
      tag.type = "application/ld+json";
      tag.innerHTML = schemaContent;
...
export const validateSeoDetails = (seoDetails, languages, defaultLangId) => {
